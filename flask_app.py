...
def api_get_video_data_from_database(video_id):
    ...
    """
    info_json = dict(requests.get(f"https://content.pinapelz.com/file/vtuber-rabbit-hole-archive/VTuber+Covers+Archive/metadata/{video_id}.info.json").json())
    if "status" in info_json.keys() and info_json["status"] == 404:
        server = create_database_connection()
        if server.check_row_exists(table_name="songs", column_name="video_id", value=video_id):
            data = server.get_query_result(f"SELECT * FROM songs WHERE video_id = '{video_id}'")
            dict_data = {}
